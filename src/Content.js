import React from 'react';
import logo from './logo.svg';
import './App.css';

function Content() {

  function clickButtonHandler(){
    console.log('Clicked!');
  }

  return (
    <main className="App-content">
      <div className="container">
        <h2>
          First app
        </h2>

        <button onClick={clickButtonHandler}>Add one</button>

        <p>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit,
          sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
          Tristique senectus et netus et malesuada fames ac turpis.
          A pellentesque sit amet porttitor eget dolor morbi non.
          Sed ullamcorper morbi tincidunt ornare massa eget egestas purus.
          Amet consectetur adipiscing elit pellentesque habitant morbi tristique senectus.
          Diam quam nulla porttitor massa id neque aliquam.
        </p>
        <p> Leo vel fringilla est ullamcorper eget nulla facilisi etiam.
          Nunc lobortis mattis aliquam faucibus purus. Congue quisque egestas diam in arcu cursus euismod quis.
          Tincidunt id aliquet risus feugiat in ante metus dictum.
          Dictum fusce ut placerat orci nulla pellentesque dignissim.
          Turpis egestas pretium aenean pharetra magna. Molestie a iaculis at erat pellentesque.
          At elementum eu facilisis sed. Euismod lacinia at quis risus sed vulputate odio.
          Mi in nulla posuere sollicitudin aliquam. Rhoncus aenean vel elit scelerisque mauris.
          Augue mauris augue neque gravida in fermentum et.
        </p>
      </div>
    </main>
  );
}

export default Content;
